...
const EditRentEstimationScreen = ({route}) => {
    ...
    const editCar = async () => {
        try {
                ...
                await uploadPhotoAsync(carDetails.image, `cars/${imageId}`)
                    .then((downloadUrl) => {
                        tempImg = downloadUrl;
                    })
                ...
                console.log(car.modelYear, '', car.make, '', car.model, ' Updated Successfully ');
                navigation.navigate('EditCarSuccess');
        }catch(error){
            alert(error.message);
        }
    }
...
export default EditRentEstimationScreen
